<template>
    <BlocksModal
        :classes="'w-full !lg:max-w-850px md:max-w-1000px !lg:(pt-55px) md:p-30px bg-no-repeat rounded-20px bg-contain bg-left-bottom'"
        :closeIconColor="'text-white'"
        @close="$emit('close')">
        <div class="font-mulish flex flex-col gap-24px">

            <div class="flex flex-col lg:flex hidden items-start gap-14px">
                <p :class="{ 'bg-red': book.Status == 'Аннулирован' }"
                    class="bg-regular-2  py-5px text-white font-bold text-10px px-10px rounded-4px">{{
                    book.Status }}</p>

                <h4 v-if="book.CityEvent" class="font-extrabold text-18px leading-22px">{{ book.Country }}, {{
                    book.CityEvent }}</h4>
                <h4 v-else class="font-extrabold text-18px">{{ book.Country }}</h4>
            </div>

            <div class="flex lg:(flex-row gap-40px) flex-col gap-20px">
                <div class="md:min-w-445px md:h-340px h-300px w-full p-24px flex justify-start items-end rounded-[16px] bg-center bg-cover"
                    style="background-image: url('/new-main-section-15/bookPlaceholder.webp');">
                    <p :class="{ 'bg-red': book.Status == 'Аннулирован' }"
                        class="bg-regular-2 lg:hidden block py-5px text-white font-bold text-10px px-10px rounded-4px">
                        {{ book.Status }}</p>
                </div>
                ...
                <div class="w-full flex flex-col gap-20px">
                    <div class="font-bold text-16px flex gap-36px">
                    ...
                    <div>
                        <div>
                            <div class="font-extrabold text-16px leading-36px">{{ priceFormatter(book.Price) }} ₽</div>
                            <p class="text-14px">без сертификата</p>
                        </div>
                    </div>
                    <div @click="$emit('openDocuments')"
