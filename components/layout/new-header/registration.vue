...
const handleSubmit = async () => {
   ...
   if (!data) return error.value = 'При регистрации возникла ошибка, попробуйте снова', updateCaptcha(), loading.value = false;

   await userStore.fetchLogin({ password: password.value, username: email.value })

   if (!userStore.userToken) return error.value = 'При регистрации возникла ошибка, попробуйте снова', updateCaptcha(), loading.value = false;
