<template>
    <div>
        ...
        <div v-if="interfaceStore.hotTours.length" class="flex flex-wrap gap-40px overflow-x-auto">
            <div class="flex gap-15px" v-for="item in interfaceStore.hotTours" :key="item.id">
                <BlocksHotTourCard :hot-tour="item" />
        ...
        <ItemEditModal :m-id="0" :show="!!idEdit" :id="idEdit" @close="idEdit = null" />
    </div>
</template>
<script setup>
...
const deleteHotTour = async id => {
    ...
    await adminStore.deleteTour(id, 0)
    await interfaceStore.getTours(0);
