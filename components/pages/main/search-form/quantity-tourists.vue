<template>
  <div>
    <v-tooltip
      ...
      <span>В этой вкладке размещены предложения только для двоих человек, если вы планируете отправиться в путешествие, в составе более 2х человек, пожалуйста, выберите предложение в вкладке "Туры"</span>
    </v-tooltip>
...
const increase = (qt) => {
  if((toursStore.quantityTourists[0].quantity + toursStore.quantityTourists[1].quantity) == 2) {
...
<style scoped>
.plus-minus {
  @apply h-32px cursor-pointer bg-black flex justify-center items-center text-white;
}
</style>