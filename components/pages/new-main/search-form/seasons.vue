<template>
   <div class="relative flex">
      <div class="input-value-new w-full" @click="$emit('open')">
         <span v-if="toursStore.selectedMonths.length" :title="value">
            {{ value }}
         </span>
         <template v-else>Выберите сезон</template>
      </div>
      <div v-if="show && toursStore.tourDates.length"
         class="bg-white absolute z-33 min-w-200px w-full left-0 scroll-style rounded-5px top-0 max-h-300px overflow-auto shadow-0 px-20px py-15px flex flex-col gap-5px">
   ...
   if (unrealMonth.value) {
      ...
      toursStore.tourDates.forEach((el) => {
         ...
         if (+el.split(".")[1] + 1 === unrealMonth.value && Object.values(lastDateOfMonth.value).includes(el))
            dates.push(`01.${unrealMonth.value}.${el.split(".")[2]}`);
      });
   } else {
...
const selectMonths = () => {
   ...
   for (let i = 0; i < toursStore.tourDates.length; i++) {
      ...
      if (+elA[0] > lastDayOfMonth && (+minMonthYear[0] === 1 && +elA[1] === 12 || +minMonthYear[0] - 1 === +elA[1])) {
            toursStore.selectedTourDates.push(toursStore.tourDates[i])
      }
